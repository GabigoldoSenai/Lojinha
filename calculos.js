...
function fn_start(){
    ...
    switch(menu){
        ...
        case 1:
            ...
            */
            debugger
            let la = {quantidade : 0, peso: 0, preco_total: 0, tipo: 0 }, espuma  , feltro , olhos , preco = {lucro_bruto: 0 , investimento: 0  , lucro_liquido: 0  , custo: 0 , tempo: 0}
            ...
            for(let n = 0; la.quantidade > n ; n ++){
                la.tipo = prompt("Qual(is) são os tipos de lã usadas?\n opções: fina, normal, grossa")
                ...
                if(la.tipo == "FINA" || la.tipo == "NORMAL" || la.tipo == "GROSSA"){
                    switch(la.tipo){
                        case "FINA":
                            la.preco= Number(prompt("Digite o peso total de lã (em g) usado: "))
            ...
            preco.tempo = Number(prompt("Insira o tempo que você levou até terminar (em h): "))
            if(preco.tempo <= 0){
                while( preco.tempo <= 0){
            ...
            preco = Math.ceil(preco.custo) + preco.tempo
            preco.lucro_bruto = preco - preco.custo
            preco.investimento = preco.lucro_bruto * 0.3
            preco.lucro_liquido = preco.lucro_bruto - preco.investimento
            

            document.write(`Custo de produção: R$${preco.custo.toFixed(2)} <br> Custo de produção +35% : R$${(preco.custo * 1.35).toFixed(2)} <br> Sugestão de preço: R$${Math.ceil(preco).toFixed(2)}<br><br> ||Lucro bruto: R$${preco.lucro_bruto.toFixed(2)} | Para investir/guardar: R$${preco.investimento.toFixed(2)} | Lucro líquido: R$${preco.lucro_liquido.toFixed(2)}`)
            break;
            ...
            case 2:
                ...
                menu_2 = Number(prompt("Os modelos padrões disponíveis são:\n 1) Boneco [M]"))
                
                switch(menu_2){
                    case 1:
                        document.write("Preço base:<br>mínimo:R$24.00   meio-termo= R$32.00    máximo:R$39.00<br> Média: R$32.00<br>preco.custo base + 35% = R$10.98<br> Lucro líquido = Lucro líquido: R$11.81 || Para investir/guardar: R$5.06")
                        break;
                    default:
                        alert("Opção não")
